...
def sender_qkd(alice, secret_key, receiver):
    ...
    for bit in secret_key:
        ...
        while not success:
            ...
            if ack is not None:
                 print('Alice\'s qubit successfully sent')
            #get a message from Bob whether he measured a "1" in rectilinear basis or a "-" in diagonal basis
            message = alice.get_classical(receiver, wait = wait_time)
            if message is not None:

                if message[0].content == 'qubit successfully acquired':
                    success = True
...
def receiver_qkd(bob, key_size, sender):
    ...
    while received_counter < key_size:
        ...
        bob_sent = bob.send_classical(sender,message_to_send, await_ack = True) 
        if bob_sent is not None:
            print('Bob successfully sent a message to Alice')
...
def check_key_sender(alice, key_check_alice, receiver):
    for bit in key_check_alice:
        ...
        while (cntr < len_check) and (eavesdropping == False):
            ...
            message_from_bob = alice.get_classical(receiver, wait = wait_time)
            if message_from_bob is not None:
                if message_from_bob[0].content == 'fail':
                    eavesdropping = True
                cntr += 1
...
def check_key_receiver(bob, key_check_bob,sender):
    ...
    while (cntr < key_check_length) and (eavesdropping == False):
        bit_from_alice = bob.get_classical(sender, wait = wait_time)
        if bit_from_alice is not None:
            if bit_from_alice[0].content == key_check_bob[cntr]:
