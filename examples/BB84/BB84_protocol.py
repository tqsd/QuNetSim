...
class BB84:
  ...
  #Bob bases
  def select_measurement(self,length):
      bases = ""
      for i in range(length):
  ...
  def send(self,sender_host,receiver_host,message):
    ...
    for q in message:
      #print(q)
      q_id, _ = sender_host.send_qubit(receiver_host.host_id, q, await_ack=True)
      ids.append(q_id)
    return ids
  ...
  def send_classically(self,host_sender,message,key_size,host_receiver):
    ...
    for class_bit in message:
      host_sender.send_classical(host_receiver.host_id,class_bit,await_ack=True)
      seq_no += 1
    return seq_no
  ...
  def receive_classically(self,host_receiver,host_sender,seq_no):
    ...
    while received_counter < seq_no:
      message = host_receiver.get_classical(host_sender.host_id, received_counter)
  ...
  def encrypt_message(self,unencrypted_string,key):
      bits = bin(int(binascii.hexlify(unencrypted_string.encode('utf8','surrogatepass')), 16))[2:]
...
def main():
    ...
    if(eve_interception):
      ...
      intercepted_message,intercepted_message_string = b.measurement(eve_bases,intercepted_qubits)
    ...
    print(seq_no)
    bases_bob_received_arr,bases_bob_received_string = b.receive_classically(host_bob,host_alice,seq_no)
    ...
    # Bob announces where they agreed on encoding and decoding bases classically.
    seq_no = b.send_classically(host_bob,bob_bases,KEY_LENGTH,host_alice)
    print(seq_no)
    Alice_received_bases,Alice_received_bases_string = b.receive_classically(host_alice,host_bob,seq_no)
    print('Alice Received Bases from Bob',Alice_received_bases_string)
    Alice_agreeing_bases = b.compare_bases(alice_bases,Alice_received_bases_string)
    print(Alice_agreeing_bases)
    
    # Step 7: Alice and Bob construct their keys
    alice_key = b.construct_key_from_indices(alice_bitstring, Alice_agreeing_bases) #Alice Side
